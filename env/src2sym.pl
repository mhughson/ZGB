    ...
    while (my $FileLine = <MainSymbolFile>) {
        ...
        if ($FileLine =~ /DEF (C\$)(\w*)\.c\$(\d*)\$.*0x(\w*).*/) {
            ...
            if ($CSourceLine) {
				...
				my $banked_addr = hex($4);
				my $bank = $banked_addr >> 16;
				my $addr = $banked_addr & hex("0xFFFF");
				printf "%02X\:%04x", $bank, $addr;
				print " ".$1.$2."\@".$CLabelSourceLineNum.":";
